...
PREDICTION_COL = "booking_status_pred_basic"
...
if PREDICTION_COL not in df.columns:
    logger.warning(f"Column {PREDICTION_COL} is absent from the dataset. Add a NULL column.")
    df = df.withColumn(PREDICTION_COL, lit(None).cast("string"))


full_table_name = f"{config.catalog_name}.{config.schema_name}.{config.batch_inference_table}"

try:
    spark.sql(f"ALTER TABLE {full_table_name} ADD COLUMNS ({PREDICTION_COL} STRING)")
    logger.success(f"✅ Column {PREDICTION_COL} added to {full_table_name}")
...
data_processor.save_to_catalog(pred_df, pred_df, write_mode="upsert", job_type="inference")
